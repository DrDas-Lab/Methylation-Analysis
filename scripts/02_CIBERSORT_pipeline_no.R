...
# ---------------------------------------------------------
expr_log <- read.table(
  "input/Expression_LAML_707_log2RPM.txt",
  header = TRUE,
  sep = "\t",
  row.names = 1,
  check.names = FALSE
...
cibersort_res <- CIBERSORT(
  ...
  mixture_file = "temp_output/CIBERSORT_mixture_RPM.txt",
  perm         = 100,
  QN           = FALSE
)
...
write.csv(
  cibersort_res,
  "temp_output/CIBERSORT_AML_results.csv"
)
...
write.csv(
  malignant_norm,
  "temp_output/CIBERSORT_AML_malignant_normalized.csv"
)

# ---------------------------------------------------------
# 12. Heatmap of AML cell states
# ---------------------------------------------------------
pdf("temp_output/CIBERSORT_AML_heatmap.pdf", width = 8, height = 6)
...
# ---------------------------------------------------------
# 13. Dominant AML state per sample
# ---------------------------------------------------------
...
write.csv(
  dominant_df,
  "temp_output/AML_dominant_cell_state.csv",
  row.names = FALSE
)
...
pdf("temp_output/CIBERSORT_AML_PCA.pdf", width = 7, height = 5)
